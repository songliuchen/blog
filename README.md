...
\\\diff-貌似express 无法通过localhost或者127.0.0.1进行代理转发，只能通过配置另外的域名进行代理
\\\diff-至于为什么配置这一步，在于设计采用前后端分离方式，考虑部署会分开进行，未避免出现跨域问题，前端接口访问都是以相对路径实现，如果前后台分开部署，需要代理进行转发后缀为.do 的请求
\\\diff-实际开发，前后台也是分成两个工程独立进行的
