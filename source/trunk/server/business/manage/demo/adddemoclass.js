...
exports.Execute=function(req,res)
{
    ...
    var sql = "insert into bt_folder_demo (Localen,Captione,NumPoste,Sortinge) select '"+linkhref+"','"+caption+ "',0,max(Sortinge)+1 from bt_folder_demo";
    dbhelper.Query(sql,function(err, rows, fields)
    {
        var result={};
        if(err)
        {
            console.log("添加分类数据失败");
            result["code"]=0;
            result["message"] = "获取数据失败！";
            var resultstring=JSON.stringify(result);
            res.end(resultstring);
