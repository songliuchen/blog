...
exports.Execute=function(req,res)
{
    ...
    var sql = "delete bt_article_story,bt_comment from bt_article_story left join bt_comment on bt_comment.parent = bt_article_story.id where bt_article_story.id="+id;
    console.log("删除故事文章数据");
    console.log(sql);
    dbhelper.Query(sql,function(err, rows, fields)
    {
        if(err)
        {
            console.log("删除故事文章数据失败");
            result["code"]=0;
            result["message"] = "获取数据失败！";
            var resultstring=JSON.stringify(result);
            res.end(resultstring);
