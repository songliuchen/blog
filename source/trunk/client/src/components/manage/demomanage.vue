		...
		<el-table :data="tableData" border style="width: 100%">
			<el-table-column prop="id" label="操作" width="150">
				<template scope="scope">
					<router-link :to="{path:'/demoedit',query: { id:scope.row.id }}"><el-button type="text" size="small">编辑</el-button></router-link>&nbsp;|&nbsp;
	...
	export default {
		...
		methods: {
			...
			loadtechnology(self,showload)
			{
				...
				$.ajax({
					type: 'POST',
					url: tszs.GetApiRootPath()+'/manage/demo/querydemolist.do',
			...
			deletetechnology(index)
			{
				if(window.confirm('你确定要取消交易吗？'))
				{
					...
					$.ajax({
						type: 'POST',
						url: tszs.GetApiRootPath()+'/manage/demo/deletedemo.do',
						contentType: "application/json; charset=utf-8",
						dataType: "json",
