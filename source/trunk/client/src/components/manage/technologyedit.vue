	...
	<div class="technologyedit">
		<label>标题:</label>
			<span>
		 	...
		 	<span>
		 		<label>属性:</label>
		 		<input type="checkbox" style="height:20px;width:20px;vertical-align: middle;" :checked="data.pinned" /> 
		 		<label style="vertical-align: middle;">置顶文章</label>
		 		<input type="checkbox" style="height:20px;width:20px;line-height:20px;vertical-align: middle;" :checked="data.recomm" />
		 		<label style="vertical-align: middle;">推荐文章</label>
		 		<input type="checkbox" style="height:20px;width:20px;line-height:20px;vertical-align: middle;"  :checked="data.closed" /> 
...
export default {
	...
	mounted: function() {
		window.editor = KindEditor.create('#content_kindeditor',{
			fileManagerJson : tszs.GetApiRootPath()+'/manage/file/filemanage.do',
			uploadJson: tszs.GetApiRootPath()+'/manage/file/uploadjson.do',
		});
	},
	methods: {
		selectClass(id){
		...
		saveEdit() {
			...
			dataparams['publish'] = tszs.IsNullOrEmpty(this.data.publish)?undefined:tszs.Replace(this.data.publish,'/','-');
			...
			if(tszs.IsNullOrEmpty(id))
			{
				$.ajax({
					type: 'POST',
					url: tszs.GetApiRootPath()+'/manage/technology/addtechnology.do',
			...
			else
			{
				...
				$.ajax({
					type: 'POST',
					url: tszs.GetApiRootPath()+'/manage/technology/updatetechnologydetail.do',
					contentType: "application/json; charset=utf-8",
					dataType: "json",
					data: JSON.stringify(dataparams),
					timeout: 5000,
	...
	beforeCreate: function() {
		...
		$.ajax({
			type: 'POST',
			url: tszs.GetApiRootPath()+'/manage/technology/technologyclass.do',
			contentType: "application/json; charset=utf-8",
			dataType: "json",
			data: JSON.stringify({'token':tszs.getCookie("token")}),
			timeout: 5000,
		...
		$.ajax({
			type: 'POST',
			url: tszs.GetApiRootPath()+'/manage/technology/querytechnologydetail.do',
			contentType: "application/json; charset=utf-8",
			dataType: "json",
