	...
	<div class="bottomnav" v-loading="showloading" element-loading-text="拼命加载中">
		<el-table :data="tableData" border style="width: 100%">
			<el-table-column prop="id" label="操作" width="150">
				<template scope="scope">
					<router-link :to="{path:'/technologyedit',query: { id:scope.row.id }}"><el-button type="text" size="small">编辑</el-button></router-link>&nbsp;|&nbsp;
		...
		<div>
			<el-card style="margin-bottom: 25px;">
				<el-pagination size="large" style="margin: 0 auto;text-align: center;" @size-change="handleSizeChange" @current-change="handleCurrentChange" :page-sizes="[10, 15, 20]" :current-page="pageindex" :page-size="pagesize" layout="total, sizes, prev, pager, next, jumper" :total="totalcount">
				</el-pagination>
			</el-card>
		</div>
	</div>
	...
	export default {
		...
		methods: {
			...
			loadtechnology(self,showload)
			{
				...
				$.ajax({
					type: 'POST',
					url: tszs.GetApiRootPath()+'/manage/technology/querytechnologylist.do',
			...
			deletetechnology(index)
			{
				if(window.confirm('你确定要取消交易吗？'))
				{
					...
					$.ajax({
						type: 'POST',
						url: tszs.GetApiRootPath()+'/manage/technology/deletetechnology.do',
						contentType: "application/json; charset=utf-8",
						dataType: "json",
