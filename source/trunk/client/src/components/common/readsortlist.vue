<template>
	<div class="readsortlist">
		<div class="title">
			<span class="titlename">排行榜</span>
		</div>
		<div class="content">
			<ul>
				<li v-for="(item, index) in data">
					<span class="sortid">{{ index+1 }}</span>
					<span class="sorttitle"><router-link :to="{path:readlist.jumpurl,query: {id:item.id}}">{{item.caption}}</router-link></span>
				</li>
			</ul>
		</div>
	</div>
...
export default {
  ...
  created: function () {
    ...
    $.ajax({
      type: 'POST',
      url: tszs.GetApiRootPath()+this.readlist.url,
      data: { },
      timeout : 5000,
